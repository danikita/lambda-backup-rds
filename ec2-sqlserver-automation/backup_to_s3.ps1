<#
.SYNOPSIS
    Performs a native (FULL, DIFFERENTIAL or LOG) backup of all user
    databases on a local SQL Server instance (EC2) and uploads the file
    to S3.
...
.DESCRIPTION
    ...
    Command from the Lambda payload):
      ...
      BACKUP_TYPE   -> FULL, DIFFERENTIAL or LOG     (default: FULL)
...
if ($BackupType -notin @("FULL", "DIFFERENTIAL", "LOG")) {
    throw "Invalid BACKUP_TYPE: $BackupType. Use FULL, DIFFERENTIAL or LOG."
}

# SQL Server type: FULL = DATABASE, DIFFERENTIAL = DATABASE ... WITH DIFFERENTIAL, LOG = LOG
$FileExt = switch ($BackupType) {
    "FULL"         { "bak" }
    "DIFFERENTIAL" { "diff.bak" }
    "LOG"          { "trn" }
}
...
"@
            ...
            # LOG backup: requires FULL or BULK_LOGGED recovery model and a
            # prior FULL backup of the same database.
            $checkRecovery = @"
SELECT recovery_model_desc
FROM sys.databases
WHERE name = '$db'
"@
            $recoveryModel = (Invoke-Sqlcmd -ServerInstance $SqlInstance -Query $checkRecovery -ErrorAction Stop).recovery_model_desc

            if ($recoveryModel -eq "SIMPLE") {
                $results += "[$BackupType][SKIP] $db -> Database is in SIMPLE recovery model, transaction log backups are not supported."
                continue
            }
